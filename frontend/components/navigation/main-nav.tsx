...
export const MainNav = () => {
  ...
  const isShipmentPage = pathname?.startsWith('/shipment/')
          ...
          <div className="flex gap-2">
            {navItems.map((item) => (
              <Button
                key={item.href}
                asChild
                variant={pathname === item.href || (item.href === '/shipment' && isShipmentPage) ? 'default' : 'ghost'}
                className={
                  pathname === item.href || (item.href === '/shipment' && isShipmentPage)
